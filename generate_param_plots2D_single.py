...
PARAM_LABELS = {
    ...
    "AMP": "Amplitude",
    "RAD": "Radius",
    "MYP": "Minimum Y Position",
    "PRD": "Period Factor",
    "XOF": "Peak/Valley CP X-Offset",
    "XMF": "Centre CP X-Offset",
    "CWT": "Curve Weight",
    "THF": "Thickness Mode: Variable",
...
class ConfigPlotData:
    ...
    thf_value: List[float] | None = None
    thf2_value: List[float] | None = None
    source_dimension: str = "2d"
...
def _extract_plot_points(
    ...
    return ConfigPlotData(
        ...
        highlight_indices,
        first_and_last_only,
        thf_value=params.thickness_factor,
        thf2_value=params.thickness_factor2,
        source_dimension=plot_dimension,
    )
...
def _format_group_title(group_name: str, subplot_idx: int) -> str:
    ...
    label_char = "B: 2D Thickness Offset"
...
def _compute_local_limits(
    ...
    def extend_entry(entry: ConfigPlotData) -> None:
        ...
        close = close_polygons and entry.source_dimension == "2d"
        xs, ys = _ensure_closed(entry.points, close=close)
        xs_all.extend(xs)
        ys_all.extend(ys)
...
def _plot_group_axes(
    ...
    close_baseline = close_polygons and baseline_aligned.source_dimension == "2d"
    ...
    for idx, variant_aligned in enumerate(aligned_variants):
        close_variant = close_polygons and variant_aligned.source_dimension == "2d"
        xs_var, ys_var = _ensure_closed(variant_aligned.points, close=close_variant)
        if not xs_var:
            continue
        ...
        if variant_aligned.name == 'THV1' or variant_aligned.name == 'THV2':
            label = f"{group_name} {label_suffix}" if label_suffix else variant_aligned.name
            control_label = None
        else:
            label = "Baseline - 2D Cross-Section"
            control_label = "CP = {CP1,CP3,CP5}" 
            # if not group_name == 'SBD':
            #     control_label = f"THV = {variant_aligned.thf_value}" 
            # else:
            #     control_label = f"THV1 = {variant_aligned.thf_value} \nTHV2 = {variant_aligned.thf2_value}"
        # label = _format_group_title(group_name, subplot_idx)
        if stacked_mode:
            color_index = stacked_position % len(color_cycle)
        else:
            color_index = idx % len(color_cycle)
        # print(variant_aligned.name)
        if variant_aligned.name == 'THV1' or variant_aligned.name == 'THV2':
            color = color_cycle[color_index]
            if variant_aligned.name == 'THV2':
                shade_2d = False
        else:
        ...
        if show_control_points:
            ...
            max_x = _max_x_coordinate(variant_aligned.control_points)
            print(max_x)
            neg_variant_control = (
                [(x-max_x, y) for x, y in variant_aligned.control_points]
                if variant_aligned.control_points
                else None
            )
            ...
            # neg_baseline_control = (
            #     [(-x, y) for x, y in baseline_aligned.control_points]
            #     if baseline_aligned.control_points
            #     else None
            # )
            # _plot_control_points(
            #     ax,
    ...
    if not stacked_mode or stacked_position == 0:
        ax.set_title(_format_group_title(group_name, subplot_idx), fontsize=18, fontweight="bold", loc="left", pad=10)
    else:
        ax.set_title("")
...
def generate_parameter_plots(
    ...
    use_1d_baseline_for_2d: bool = False,
) -> None:
    ...
    Set use_1d_baseline_for_2d to True to draw the baseline curve from the 1D geometry while still plotting 2D variants.
    """
    ...
    if use_1d_baseline_for_2d and plot_dimension != "2d":
        raise ValueError("--use-1d-baseline requires --plot-dimension set to '2d'.")
    ...
    baseline_labels = {"baseline", "baseline7"}
    for row in rows:
        # Identify the prototype id regardless of column case
        prototype_id_value = ""
        for key, value in row.items():
            if isinstance(key, str) and key.lower() == "prototype id":
                prototype_id_value = str(value).strip()
                break

        entry_dimension = plot_dimension
        if (
            use_1d_baseline_for_2d
            and plot_dimension == "2d"
            and prototype_id_value.lower() in baseline_labels
        ):
            entry_dimension = "1d"
...
def main() -> None:
    ...
    parser.add_argument(
        "--use-1d-baseline",
        action="store_true",
        help="When plotting 2D cross-sections, use the 1D curve for the baseline overlay.",
    )
    ...
    generate_parameter_plots(
        ...
        use_1d_baseline_for_2d=args.use_1d_baseline,
    )
